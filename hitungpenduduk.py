...
cur = db.cursor()
cur.execute("SELECT id,nama FROM `statistikpenduduk` WHERE id LIKE '%3327%' ")
...
for a in cekdesa:
    ...
    """Insert to Database"""
    cur.execute("UPDATE statistikpenduduk SET total_kk='"+hasilkk+"',total_ak='"+hasilak+"',total_penduduk='"+hasilpenduduk+"' WHERE id='"+a[0]+"' ")
    db.commit()
